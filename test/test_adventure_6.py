...
# Test 3: Function calculate_average_weight_by_material - Material Present
def test_calculate_average_weight_material_present(sample_df):
    """
    Tests calculate_average_weight_by_material for a material that exists
    in the dataset ('Gold').
    """
    material = 'Gold'
    result_avg_weight = adventure.calculate_average_weight_by_material(sample_df, material)

    # Manually calculate average weight for Gold: (0.1 + 0.8 + 0.4 + 0.3 + 2.5) / 5 = 4.1 / 5 = 0.82
    expected_avg_weight = 0.82

    assert isinstance(result_avg_weight, float), "Return type should be float"
    assert result_avg_weight == pytest.approx(expected_avg_weight)
